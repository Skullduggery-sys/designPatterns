#if 0
Данный подход требует хранения объекта, над которым будет выполнена команда
в самой команде, предоставляя, по сути, отложенный вызов некоторого действия.

Может использоваться тогда, когда мы заранее уверены в том, что потребуется
некоторое действие и ожидаем указания выполнить команду.
#endif
